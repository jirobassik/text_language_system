...
from apps_text.app_summarize.api.schems import Summarize, SummarizeFile
...
def summarize_api_text(request, sum_text_schem: Summarize):
    res = methods.get(sum_text_schem.method.name)(
        sum_text_schem.text, sum_text_schem.num_sentences
    )
    return {"result": res}
...
@throttle(User60MinRateThrottle, User100PerDayRateThrottle)
def summarize_api_file(
    request, sum_text_file_schem: Form[SummarizeFile], file: File[UploadedFile]
):
